...
interface AnyMoeNFTAuctionInterface {
    event TransferIn(address owner, uint64 tokenId, uint256 amount);
    event TransferOut(address destination, uint256 tokenId, uint256 amount);
    event CreateAuction(uint256 auctionId, address owner, uint64 tokenId, uint256 amount, uint baseBid, uint bidIncrement, uint duration);
    ...
    function createAuction(uint64 tokenId, uint256 amount, uint baseBid, uint bidIncrement, uint duration);
